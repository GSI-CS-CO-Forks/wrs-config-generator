...
class Encoder_6_0_0(Encoder_5_0):
    ...
    def getGlobalConfig(self, gblConfig):
        ...
        if enabled :
            # Set drift to 1 second and check every half hour
            lines.append(self.buildEntry(self.getItem("CONFIG_SNMP_SYSTEM_CLOCK_DRIFT_THOLD","1")))
            lines.append(self.buildEntry(self.getItem("CONFIG_SNMP_SYSTEM_CLOCK_UNIT_HOURS","n")))
    ...
    def _getVLansPorts(self, ports):
        ...
        for vlan_port_data in ports:
            ...
            lines.append(self.buildEntry(
                    self.getItem(prefix+"_LLDP_TX_VID", vlanPortLldpTxVid if vlanPortLldpTxVid!=None else "2586",item.itemTypeInt)))
